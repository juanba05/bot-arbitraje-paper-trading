...
Fuentes de datos (en orden de prioridad):
  ...
  2. IOL /Mercado/GetCaucionPuntas → tasa TNA real via scraping web con sesion cookies
  3. Ultimo registro en DB  → fallback si todo lo demas falla

Logica financiera real:
  - Ganancia = Monto × (TNA/100) × (plazo/365)
  - Comision = Monto × 0.0163% × 1.21 (IVA) — verificado Tx 167032333
...
log_caucion = logging.getLogger("motor_cauciones")
...
def _es_fin_rueda_caucion(ahora):
    """Ventana de cierre (últimos 5 min): 16:55-17:00."""
    return (ahora.hour == 17) or (ahora.hour == 16 and ahora.minute >= 55)
...
def crear_tabla_cauciones_simuladas():
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Error creando tabla: {e}")
...
def _crear_sesion_web_iol():
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Login web IOL fallido: {e}")
        return None
    ...
    for intento in range(1, MAX_INTENTOS + 1):
        try:
            ...
            if resp.status_code in (401, 403):
                # Sesion expirada en IOL — invalidar cache y reintentar
                _sesion_web_cache["sesion"] = None
                _sesion_web_cache["ts"] = None
                raise ValueError(f"HTTP {resp.status_code} — sesion expirada, se forzara re-login")

            if resp.status_code != 200:
                raise ValueError(f"HTTP {resp.status_code}")

            # Si IOL devuelve HTML en lugar de JSON, la sesion expiró (redirect a login)
            content_type = resp.headers.get("Content-Type", "")
            if "text/html" in content_type:
                _sesion_web_cache["sesion"] = None
                _sesion_web_cache["ts"] = None
                raise ValueError("Respuesta HTML en lugar de JSON — sesion expirada, se forzara re-login")
            ...
            try:
                ...
                log_caucion.info(f"[CAUCIONES] ⚠️  Formato de tasa inesperado: {tasa_raw!r} — {parse_err}")
                return None
            ...
            if intento < MAX_INTENTOS:
                log_caucion.info(f"[CAUCIONES] ⚠️  IOL web scraper error (intento {intento}/{MAX_INTENTOS}): {e} — reintentando en {ESPERA_ENTRE_INTENTOS}s...")
                _time.sleep(ESPERA_ENTRE_INTENTOS)
            else:
                log_caucion.info(f"[CAUCIONES] ⚠️  IOL web scraper error (intento {intento}/{MAX_INTENTOS}): {e} — sin mas intentos.")
    return None
...
def _obtener_tasa_db(plazo_dias=1):
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Error leyendo tasa de DB: {e}")
        return None, None
...
def _parsear_saldo_iol(datos):
    ...
    NOTA: logica identica en ejecutor_paper._parsear_saldo_ars_iol().
    Si IOL cambia el formato de /estadocuenta, actualizar ambas funciones.
    """
    if datos is None:
        return None
    ...
    if IOL_DISPONIBLE:
        try:
            ...
            if saldo is not None and saldo >= 0:
                ...
                log_caucion.info(f"[CAUCIONES] ℹ️  IOL respondio pero no pudo parsear saldo. Datos: {str(datos)[:200]}")
        except Exception as e:
            log_caucion.info(f"[CAUCIONES] ⚠️  Error consultando IOL para saldo: {e}")
...
def calcular_capital_a_usar(saldo_disponible, ahora=None):
    ...
    if (17 <= ahora.hour < 18) or (ahora.hour == 16 and ahora.minute >= 55):
        pct = CAPITAL_CIERRE_PCT
        desc = "Modo cierre (16:55-17:00) → 100% del capital"
    else:
...
def registrar_intento_orden_real_caucion(
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] Error registrando intento real: {e}")
        return None
...
def actualizar_orden_real_caucion(
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] Error actualizando orden real: {e}")
        return False
...
def registrar_fill_real_caucion(
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] Error registrando fill real: {e}")
        return False
...
def registrar_operacion_real_caucion(plazo_dias, tna_mercado, monto_colocado, comision_real, ganancia_neta_real, fuente="IOL_REAL"):
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] Error registrando operacion real: {e}")
        return False
...
def obtener_promedio_historico(hora_franja, plazo_dias=None, dia_semana=None):
    """
    Promedio de tasas IOL_WEB en la misma franja horaria.
    Si se pasa plazo_dias, filtra solo ese plazo — cada plazo tiene su historico propio.
    Si se pasa dia_semana (0=lunes..6=domingo Python), intenta comparar mismo dia de semana
    primero; si no hay suficientes muestras, cae a cualquier dia.

    Cadena de fallback:
      1. mismo hour + mismo plazo + mismo dia_semana (7 dias)
      2. mismo hour + mismo plazo + mismo dia_semana (30 dias)
      3. mismo hour + mismo plazo (30 dias, cualquier dia)   ← comportamiento original
    """
    MIN_MUESTRAS = 3

    # Convertir weekday Python (0=lun) a strftime('%w') SQLite (0=dom, 1=lun...)
    sqlite_dow = str((dia_semana + 1) % 7) if dia_semana is not None else None

    def _query(cur, hora, plazo, ventana_dias, dow=None):
        h      = f"{hora:02d}"
        filtro_dow   = "AND strftime('%w', timestamp) = ?" if dow else ""
        filtro_plazo = "AND plazo_dias = ?" if plazo is not None else ""
        params = [h]
        if plazo is not None:
            params.append(int(plazo))
        params.append(f"-{ventana_dias}")
        if dow:
            params.append(dow)
        cur.execute(f"""
            ...
            WHERE strftime('%H', timestamp) = ?
              {filtro_plazo}
              AND tna_mercado > 0 AND tna_mercado < 300
    ...
    def _suficiente(res):
        return res and res[0] is not None and res[1] >= MIN_MUESTRAS
    ...
    try:
        ...
        res = None
        if sqlite_dow:
            # Intento 1: mismo dia de semana, ventana 7 dias
            res = _query(cur, hora_franja, plazo_dias, 7, dow=sqlite_dow)
            if not _suficiente(res):
                # Intento 2: mismo dia de semana, ventana 30 dias
                res = _query(cur, hora_franja, plazo_dias, 30, dow=sqlite_dow)
        if not _suficiente(res):
            # Intento 3 (fallback): cualquier dia, ventana 7 dias
            res = _query(cur, hora_franja, plazo_dias, 7)
        if not _suficiente(res):
            # Intento 4: cualquier dia, ventana 30 dias
            res = _query(cur, hora_franja, plazo_dias, 30)

        conn.close()
        return round(res[0], 2) if _suficiente(res) else None
    except Exception as e:
        log_caucion.info(f"[CAUCIONES] Error leyendo historial: {e}")
        return None
...
def _purge_historial_viejo(cur, dias_retener=60):
    """Elimina registros de cauciones_simuladas con mas de N dias. Operacion rapida."""
    try:
        cur.execute("""
            DELETE FROM cauciones_simuladas
...
def guardar_simulacion(datos_dict):
    ...
    try:
        ...
        cur = conn.cursor()
        _purge_counter += 1
        if _purge_counter % 200 == 0:
            _purge_historial_viejo(cur, dias_retener=60)
        cur.execute("""
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Error guardando simulacion: {e}")
        return False
...
def guardar_senal_db(tna, promedio, desviacion_pct, capital, ganancia_neta, plazo, vencimiento):
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Error guardando senal: {e}")
        return False
...
def guardar_tasa_historica(tna, fuente, plazo_dias=1):
    ...
    try:
        ...
        log_caucion.info(f"[CAUCIONES] ⚠️  Error guardando tasa historica: {e}")
...
def _umbral_adaptativo(plazo_dias):
    """
    Devuelve el umbral de desviacion efectivo para el plazo dado.
    Si no hubo seniales en los ultimos 7 dias para ese plazo, reduce el umbral
    a la mitad (minimo 5%) para ser menos exigente en mercados estables.
    El umbral normal nunca baja de su valor configurado si hay historial de seniales.
    """
    try:
        conn = sqlite3.connect(RUTA_DB)
        cur = conn.cursor()
        cur.execute("""
            SELECT COUNT(*) FROM cauciones_simuladas
            WHERE plazo_dias = ?
              AND tiene_senal = 1
              AND fuente_tasa = 'IOL_WEB'
        ...
        conn.close()
        if n == 0:
            reducido = max(5.0, UMBRAL_CAUCION / 2.0)
            log_caucion.info(f"  [UMBRAL] Plazo {plazo_dias}d: sin seniales en 7d — umbral reducido {UMBRAL_CAUCION}% → {reducido}%")
            return reducido
    except Exception:
        pass
    return UMBRAL_CAUCION


def analizar_cauciones(modo="auto", seniales_hoy=0):
    """
    ...
    log_caucion.info(f"\n{'='*60}")
    log_caucion.info(f"  SIMULADOR DE CAUCIONES - {ahora.strftime('%d/%m/%Y %H:%M:%S')}")
    ...
    # Ventanas de cierre
    # 16:00-17:00 → fallback 1d: disparar si ganancia > 0 aunque TNA no sea excepcional
    # 16:30-17:00 → evaluar 7d si tiene disparo excepcional vs su propio historial
    es_cierre_previo = (ahora.hour == 16)
    es_ventana_7d    = (ahora.hour == 16 and ahora.minute >= 30)

    log_caucion.info(f"  Plazo optimo:   {plazo_optimo} dia(s) - {desc_plazo}")
    log_caucion.info(f"  Plazos a eval.: {plazos_disponibles}")
    log_caucion.info(f"  Fin de rueda:   {'SI' if fin_rueda else 'NO'}")

    saldo, fuente_saldo, _ = obtener_saldo_disponible()
    capital, pct_capital, desc_capital = calcular_capital_a_usar(saldo, ahora)
    log_caucion.info(f"  Saldo IOL:      ARS {saldo:,.2f} ({fuente_saldo})")
    log_caucion.info(f"  Capital a usar: ARS {capital:,.2f} ({pct_capital*100:.0f}%) - {desc_capital}")
    ...
    log_caucion.info(f"  Comision usada: {comision_pct_aplicada:.4f}% ({origen_comision})")
    ...
    for plazo in plazos_disponibles:
        ...
        if tna_raw is None:
            msg_no_tasa = f"Sin tasa IOL para plazo {plazo}d (IOL no ofrece o API sin datos)"
            log_caucion.info(f"\n  [NO] {msg_no_tasa}")
            log_caucion.info(f"  [plazo {plazo}d] {msg_no_tasa}")
            continue

        tna = tna_raw
        fuente_tna = "IOL_WEB"
        log_caucion.info(f"  [plazo {plazo}d] TNA obtenida: {tna:.2f}%")
        if tna < TNA_MINIMA or tna > TNA_MAXIMA_VALIDA:
            log_caucion.info(f"  [plazo {plazo}d] TNA {tna:.2f}% fuera de rango [{TNA_MINIMA}, {TNA_MAXIMA_VALIDA}] — descartada")
            continue
        ...
        # Historico propio del plazo — nunca mezcla 1d con 2d ni con 3d
        promedio = obtener_promedio_historico(ahora.hour, plazo_dias=plazo, dia_semana=ahora.weekday())
        if promedio is not None and promedio > 0:
            desviacion_pct = round(((tna - promedio) / promedio) * 100, 1)
        else:
            desviacion_pct = 0.0

        # Umbral adaptativo: si no hubo seniales en 7 dias para este plazo, baja a la mitad
        umbral_efectivo = _umbral_adaptativo(plazo)
        ...
        elif fin_rueda and plazo == 1:
            # Ventana de cierre (16:50-17:00): ejecutar 1d si hay cualquier ganancia positiva.
            # Preferible una ganancia minima que dejar el capital sin rendimiento toda la noche.
            tiene_senal = True
            motivo_no_senal = ""
        elif sin_historial:
            # Sin historial aun: acumular datos pero no ejecutar.
            # Con la comision corregida ganancia > 0 casi siempre, por lo que
            # disparar sin referencia historica seria operar sin criterio de calidad.
            tiene_senal = False
            motivo_no_senal = (
                f"Sin historial {plazo}d para esta hora — acumulando datos, "
                f"TNA {tna:.1f}% neta ARS {rend['ganancia_neta']:.2f}"
            )
        elif desviacion_pct >= umbral_efectivo:
            tiene_senal = True
            motivo_no_senal = ""
        else:
            tiene_senal = False
            motivo_no_senal = (
                f"Desviacion {desviacion_pct:.1f}% menor al umbral {umbral_efectivo}% "
                f"sobre promedio {plazo}d: {promedio:.1f}%"
            )
    ...
    # 7d: se evalua en tres casos:
    #   1. Salto extraordinario vs curva 1-3d en cualquier momento (logica original)
    #   2. Ventana 30min antes del cierre (16:30-17:00) — dispara solo si es
    #      excepcional comparado con el historico PROPIO de 7d (no mezcla con 1-3d)
    #   3. Fallback: IOL no ofrece plazos 1-3d (feriado / fin de semana largo)
    #      En este caso se evalua con el mismo criterio que cualquier plazo normal.
    max_tna_1_3 = None
    ...
    if usar_7d and tna_7d is not None and 0 < tna_7d < TNA_MAXIMA_VALIDA:
        ...
        # Historico PROPIO de 7d — nunca se compara con tasas de 1d, 2d o 3d
        promedio_7d = obtener_promedio_historico(ahora.hour, plazo_dias=7, dia_semana=ahora.weekday())
        if promedio_7d is not None and promedio_7d > 0:
            desviacion_7d = round(((tna_7d - promedio_7d) / promedio_7d) * 100, 1)
        else:
            desviacion_7d = 0.0

        sin_historial_7d = (promedio_7d is None)
        umbral_7d = _umbral_adaptativo(7)
        ...
        if cap_min_7d is None:
            ...
            # Ventana de cierre 16:30: disparar si excepcional vs historico propio,
            # o si no hay historial aun y hay ganancia positiva y no hubo señales hoy.
            if not sin_historial_7d and desviacion_7d >= umbral_7d:
                tiene_senal_7d = True
                motivo_7d = (
                    f"7d ventana cierre: disparo excepcional TNA {tna_7d:.1f}% "
                    f"vs historico 7d {promedio_7d:.1f}% (desv {desviacion_7d:+.1f}%)"
                )
            else:
                tiene_senal_7d = False
                if sin_historial_7d:
                    motivo_7d = (
                        f"7d ventana cierre: sin historial propio — acumulando datos, "
                        f"TNA {tna_7d:.1f}% neta ARS {rend_7d['ganancia_neta']:.2f}"
                    )
                else:
                    motivo_7d = (
                        f"7d ventana cierre: TNA {tna_7d:.1f}% no excepcional vs historico 7d "
                        f"{promedio_7d:.1f}% (desv {desviacion_7d:+.1f}%, umbral {umbral_7d}%)"
                    )
        elif _es_salto_extraordinario_7d(max_tna_1_3, tna_7d):
            # Fuera de ventana: 7d tiene tasa excepcional vs curva 1-3d
            tiene_senal_7d = True
            motivo_7d = "7d habilitado por salto extraordinario vs curva 1-3d"
        else:
            # Fallback: IOL no ofrece plazos 1-3d (feriado / fin de semana largo)
            # Se evalua con el mismo criterio que cualquier plazo normal
            if sin_historial_7d:
                tiene_senal_7d = False
                motivo_7d = (
                    f"7d unico disponible (IOL sin 1-3d): sin historial propio — "
                    f"acumulando datos, TNA {tna_7d:.1f}% neta ARS {rend_7d['ganancia_neta']:.2f}"
                )
            elif desviacion_7d >= umbral_7d:
                tiene_senal_7d = True
                motivo_7d = (
                    f"7d unico disponible: TNA {tna_7d:.1f}% vs historico {promedio_7d:.1f}% "
                    f"(desv {desviacion_7d:+.1f}%, umbral {umbral_7d}%)"
                )
            else:
                tiene_senal_7d = False
                motivo_7d = (
                    f"7d unico disponible: TNA {tna_7d:.1f}% no excepcional vs historico "
                    f"{promedio_7d:.1f}% (desv {desviacion_7d:+.1f}%, umbral {umbral_7d}%)"
                )
    ...
    log_caucion.info(f"\n  {'-'*50}")
    log_caucion.info("  ANALISIS POR PLAZO:")
    log_caucion.info(f"  {'-'*50}")

    if not resultados_por_plazo:
        log_caucion.info("  [NO] Sin datos de tasa para ningun plazo.")
        log_caucion.info(f"{'='*60}")
    ...
    # Cascade eliminado: con comision corregida ganancia_neta > 0 es casi siempre verdadero
    # y forzar una ejecucion al cierre sin criterio de desviacion historica no agrega valor.
    # Solo se opera si hubo disparo genuino (desviacion >= umbral) en algun plazo 1-3d.

    if mejor_plazo is None or mejor_plazo not in resultados_por_plazo:
        return {"puede_operar": False, "motivo": "Sin plazo valido tras evaluacion"}
    ...
    log_caucion.info(f"\n  {'-'*50}")
    log_caucion.info(f"  MEJOR PLAZO: {plazo} dia(s) - vence {vencimiento}")
    log_caucion.info(f"  {'-'*50}")
    log_caucion.info(f"  Fuente tasa:      {fuente_tna}")
    log_caucion.info(f"  TNA mercado:      {tna:.2f}%")
    log_caucion.info(f"  Capital colocado: ARS {rend['monto_colocado']:>12,.2f}")
    log_caucion.info(f"  Ganancia bruta:   ARS {rend['ganancia_bruta']:>12,.2f}")
    log_caucion.info(f"  Comision IOL:     ARS {rend['comision_iol']:>12,.2f}")
    log_caucion.info(f"  Ganancia NETA:    ARS {rend['ganancia_neta']:>12,.2f}")
    log_caucion.info(f"  Al vencimiento:   ARS {rend['monto_al_vencimiento']:>12,.2f}")
    log_caucion.info(f"  Rentabilidad:     {rend['rentabilidad_pct']:.4f}%")
    log_caucion.info(f"  G/Dia bloqueo:    ARS {mejor['ganancia_neta_por_dia_bloqueo']:>12,.4f}")
    prom_str = f"{promedio:.2f}% TNA" if promedio else "sin historial aun"
    log_caucion.info(f"  Prom. historico:  {prom_str}")
    log_caucion.info(f"  Desviacion:       {desviacion_pct:+.1f}%")
    if capital_minimo_requerido is None:
        log_caucion.info("  Cap. minimo:      Inviable para cualquier capital (costos actuales)")
    else:
        log_caucion.info(f"  Cap. minimo:      ARS {capital_minimo_requerido:>12,.2f}")
        log_caucion.info(f"  Cap. faltante:    ARS {capital_faltante:>12,.2f}")
    if media_capital_min is not None:
        log_caucion.info(f"  Media cap. min 7d: ARS {media_capital_min:,.2f} ({muestras_media_capital} muestras)")
    ...
    if tna_7d is not None:
        log_caucion.info(f"  Regla 7d:        {'HABILITADO' if usar_7d else 'DESCARTADO'} (TNA 7d={tna_7d:.2f}%)")

    estado_senal = semaforo(desviacion_pct, tiene_senal)
    log_caucion.info(f"  Estado:           {estado_senal}")

    if not tiene_senal:
        log_caucion.info(f"\n  [INFO] Sin senal: {motivo_no_senal}")
    else:
        log_caucion.info(f"\n  [SENAL] Tasa {tna:.1f}% TNA | Plazo {plazo}d | Ganancia ARS {rend['ganancia_neta']:.2f}")
        log_caucion.info(f"          Colocar ARS {capital:,.0f} | Vence {vencimiento}")
    ...
    if tiene_senal:
        ...
        if ok:
            log_caucion.info("  [OK] Senal guardada en base de datos.")
...
if __name__ == "__main__":
    ...
    log_caucion.info("Iniciando motor_cauciones.py - modo real estricto (solo IOL web)")
    resultado = analizar_cauciones(modo="real")

    if resultado:
        log_caucion.info(f"\nResumen:")
        log_caucion.info(f"  Puede operar: {resultado.get('puede_operar')}")
        log_caucion.info(f"  Tiene señal:  {resultado.get('tiene_senal')}")
        if resultado.get("puede_operar"):
            log_caucion.info(f"  Mejor plazo:  {resultado.get('plazo')}d | TNA {resultado.get('tna')}% | Ganancia ARS {resultado.get('ganancia_neta')}")
